...
def do_get(type, coin):
    url = "http://api.btctrade.com/api/{}?coin={}".format(type, coin)
...
def do_post(type, data):
    ...
    signature = hmac.new(hashlib.md5(manege_keys(1)).hexdigest(), data,
...
def orders(coin, type):
    data = 'coin={}&type={}&since={}&ob=ASC&key={}&nonce={}&version=2'.format(
           coin, type, 1493695903.525188, manege_keys(0), gen_nonce())
...
def sell_or_buy(method, coin, amount, price):
    data = 'coin={}&amount={}&price={}&key={}&nonce={}&version=2'.format(
           coin, amount, price, manege_keys(0), gen_nonce())
